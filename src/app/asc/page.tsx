...
export default function QuoteBuilder() {
  ...
  // Water filter add-on — pick metro/regional cost, apply ASC discount (no STC)
  ...
  const waterFilterAscDiscount = includesWaterFilter && selectedWaterFilter ? selectedWaterFilter.asc_discount : 0
  ...
  // Total ASC discount = base + HWHP + HVAC + water filter + upgrade (sum, since ASC amounts can be negative → increases price)
  const inboundDiscount = baseInboundDiscount + hwhpAscDiscount + hvacAscDiscount + waterFilterAscDiscount + upgradeAscDiscount
  ...
  // the real total/fortnightly above (cash delta + its own ASC discount, same as everything else on
  // this page), so they can never drift from what the quote would actually show.
  ...
  const hwhpPromo = cheapestHwhp ? (() => {
    ...
    return {
      ...
      ...previewAddOnDelta(cost - cheapestHwhp.stc_value - comboDiscount, cheapestHwhp.asc_discount),
    }
  })() : null

  const hvacPromo = cheapestHvac ? (() => {
    const cost = (territory === 'Regional' ? cheapestHvac.cost_regional : cheapestHvac.cost_metro) ?? 0
    return previewAddOnDelta(cost, cheapestHvac.asc_discount)
  })() : null
  ...
  const saveQuote = async () => {
    ...
    if (!matchedPackage && !_isHwhpOnly && !isWaterFilterOnly) {
