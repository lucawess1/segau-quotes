...
export default function AdminPage() {
  ...
  const [snapshotResult, setSnapshotResult] = useState<{ snapshot_id: number; label: string; row_count: number } | null>(null)
  ...
  const takeSnapshot = async () => {
    ...
    const { data, error } = await supabase.rpc('take_packages_snapshot', {
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl p-4 mb-4">
          <div className="flex items-start gap-2 mb-3">
            ...
            <div className="flex-1">
              <p className="font-medium text-sm">Take packages snapshot</p>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                Capture the current state of active packages with a label. Useful before structural changes (new product rollouts, retiring old products).
              </p>
            </div>
          </div>
            ...
            {snapshotResult && (
              <div className="px-3 py-2 rounded-md bg-green-50 dark:bg-green-950/50 border border-green-200 dark:border-green-800 flex items-start gap-2">
                ...
                <p className="text-xs text-green-800 dark:text-green-300">
                  Snapshot <span className="font-mono">#{snapshotResult.snapshot_id}</span> captured — {snapshotResult.row_count.toLocaleString()} package{snapshotResult.row_count === 1 ? '' : 's'}
                </p>
              </div>
            )}
          </div>
