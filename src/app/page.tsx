...
export default function QuoteBuilder() {
  ...
  // the real total/fortnightly above (not a fixed advertised rate), so they can never drift from
  // what the quote would actually show if the add-on were selected.
  const previewAddOnDelta = (extraCashDelta: number) => {
    const hypAfterStc = Math.max(0, basePriceAfterStc + (addOnNetCash + extraCashDelta) / financeMultiplier)
  ...
  const hwhpPromo = cheapestHwhp ? (() => {
    ...
    return { targetProductSet: HWHP_ADD_MAP[productSet], ...previewAddOnDelta(cost - cheapestHwhp.stc_value - comboDiscount) }
  })() : null
