...
// Base product mapping — used for composable HWHP add-on flow
...
export default function QuoteBuilder() {
  ...
  // Water filter is available two ways: as a toggle alongside any other product (like HVAC), or
  // as a standalone 'Water Filter Only' product type, which forces it on.
  const isWaterFilterOnly = productSet === 'Water Filter Only'
  const includesWaterFilter = isWaterFilterIncluded || isWaterFilterOnly

  // Whether HWHP is combined with a non-HWHP base (used for $600 combo discount rule)
  ...
  useEffect(() => {
    if (includesWaterFilter && waterFilterProducts.length > 0 && !selectedWaterFilterId) setSelectedWaterFilterId(waterFilterProducts[0].id)
  ...
  // Finance-term math is applied uniformly afterwards (dividing by 0.80 or 0.70 for 60m/84m).
  // All add-on discounts are dollar amounts subtracted from cash-after-STC just like the base discount.
  ...
  // HWHP add-on — pick metro/regional cost, apply own STC & inbound discount
  ...
  const hwhpInboundDiscount = includesHwhp && selectedHwhp ? selectedHwhp.inbound_discount : 0

  // HVAC add-on — pick metro/regional cost, apply inbound discount (no STC)
  ...
  const hvacInboundDiscount = includesHvac && selectedHvac ? selectedHvac.inbound_discount : 0

  // Water filter add-on — pick metro/regional cost, apply inbound discount (no STC)
  ...
  const waterFilterInboundDiscount = includesWaterFilter && selectedWaterFilter ? selectedWaterFilter.inbound_discount : 0
  ...
  // Inverter upgrade: flat $ on top, own inbound discount. STC unaffected (depends on panels+battery only).
  const upgradeCost = selectedInverterUpgrade?.price_upgrade ?? 0
  const upgradeInboundDiscount = selectedInverterUpgrade?.inbound_discount ?? 0

  // Combined cash-after-STC (before inbound discount adjustments):
  //   base_cash_after_stc + hwhp_cost - hwhp_stc + hvac_cost + water_filter_cost + upgrade_cost - combo_discount
  const combinedCashAfterStcPreDiscount =
    baseCashAfterStc + (hwhpCost - hwhpStc) + hvacCost + waterFilterCost + upgradeCost - comboDiscount

  // Total inbound discount = base + HWHP + HVAC + water filter + upgrade discounts
  const inboundDiscount = baseInboundDiscount + hwhpInboundDiscount + hvacInboundDiscount + waterFilterInboundDiscount + upgradeInboundDiscount

  // Kept for backward compatibility of downstream code (some UI shows stc separately)
  ...
  // Fortnightly scaling — proportional to how much cash moved from raw variant price to composed price
  ...
  // composed on the cash basis (product cost less its own inbound discount) and then finance-uplifted,
  ...
  const salesforceProducts = {
    solarBattery: baseStc + (baseCashAfterStc - baseInboundDiscount + upgradeCost - upgradeInboundDiscount) / financeMultiplier,
    hwhp: includesHwhp ? hwhpStc + (hwhpCost - hwhpStc - hwhpInboundDiscount) / financeMultiplier : 0,
    hvacSplit: includesHvac && !hvacIsDucted ? (hvacCost - hvacInboundDiscount) / financeMultiplier : 0,
    ducted: includesHvac && hvacIsDucted ? (hvacCost - hvacInboundDiscount) / financeMultiplier : 0,
    waterFilter: (waterFilterCost - waterFilterInboundDiscount) / financeMultiplier,
  }
